...
(function(undefined) {
  ...
  var Kalendae = function(targetElement, options) {
    ...
    var self = this,
        ...
        vsd, columnHeaders = [],
        $cal, $title, $caption, $header, $days, dayNodes = [],
        $span, i = 0,
        j = opts.months;
  ...
  Kalendae.prototype = {
    defaults: {
      ...
      /* format mask for month titles. See momentjs.com for rules */
      dayNumberFormat: 'D',
    ...
    directions: {
      'past': function(date) {
        return moment(date).yearDay() >= today.yearDay();
      },
    ...
    draw: function draw() {
      // return;
      var month = moment(this.viewStartDate).hours(12),
           //force middle of the day to avoid any weird date shifts
          day, classes = this.classes,
          cal, $span, klass, i = 0,
          c, j = 0,
          k, s, dateString, opts = this.settings;
...
*/
  ...
  var MinPubSub = function(d) {
    ...
    d.subscribe = function( /* String */ topic, /* Function */ callback, /* Boolean */ topPriority) {
      ...
      if (topPriority) cache[topic].push(callback);
      else
  ...
  var moment = Kalendae.moment = (function(Date, undefined) {
    ...
    var moment, round = Math.round,
        ...
        i, charactersToReplace = /(\[[^\[]*\])|(\\)?(Mo|MM?M?M?|Do|DDDo|DD?D?D?|dddd?|do?|w[o|w]?|YYYY|YY|a|A|hh?|HH?|mm?|ss?|zz?|ZZ?|LT|LL?L?L?)/g,
    ...
    moment.fn = Moment.prototype = {
      ...
      day: function(input) {
        ...
        return (typeof input === 'undefined') ? day : this.add({
          d: input - day
        });
      }
    };
  ...
  moment.fn.yearDay = function(input) {
    ...
    return (typeof input === 'undefined') ? yearday : this.add({
      d: input - yearday
    });
  }
    ...
    jQuery.fn.kalendae = function(options) {
      this.each(function(i, e) {
        if (e.tagName === 'INPUT') {
          ...
          //otherwise, insert a flat calendar into the element.
          $(e).data('kalendae', new Kalendae($.extend({}, {
            attachTo: e
          }, options)));
        }
      });
